...
//   Core 1:
//     net_task        - Pops one utterance, POSTs raw PCM to /transcribe,
//                       parses transcript, pushes onto txt_queue. Higher
//                       priority than merge_task so slots free promptly.
//
...
//
// Arduino IDE -- MUST set a large app partition (Sketch too big ~1.3 MB limit):
//   Tools -> Partition Scheme -> "Huge APP (3 MB No OTA / 1 MB SPIFFS)"
//   (Or "Minimal SPIFFS (1.9 MB APP ...)") so the firmware (~1.7 MB+) fits.
//   Default scheme only gives ~1.25 MB app; Wi-Fi + TLS + Bluedroid + this
//   sketch exceed that.
...
// Arduino IDE injects function prototypes after includes; readFrame() uses a
// FrameStats* before the full struct definition later in this file — without
// this forward declaration you get: 'FrameStats' has not been declared.
struct FrameStats;

// Thread-safe USB serial. Bluedroid invokes the SSID filter from its own task
// while setup()/VAD/net/merge print; interleaved Serial.* shows mojibake and
// can overflow the CDC TX buffer. Log only BT *matches* to avoid scan floods.
static SemaphoreHandle_t g_log_mtx = nullptr;

static void log_init(void) {
  if (!g_log_mtx) g_log_mtx = xSemaphoreCreateMutex();
}

static inline void log_lock(void) {
  if (g_log_mtx) (void)xSemaphoreTake(g_log_mtx, portMAX_DELAY);
}

static inline void log_unlock(void) {
  if (g_log_mtx) xSemaphoreGive(g_log_mtx);
}
...
static void log_bt_match(const char* ssid, int rssi) {
  if (!g_log_mtx) return;
  if (xSemaphoreTake(g_log_mtx, pdMS_TO_TICKS(150)) != pdTRUE) return;
  Serial.printf("[BT scan] MATCH \"%s\" RSSI=%d\n", ssid, rssi);
...
// Bluetooth speaker matching (ESP32-A2DP):
// Default `start("name")` only accepts names where the *first* strlen(name)
// characters equal the advertised name — so "Stone Stone 350 Pro" does NOT match
// "boAt Stone Stone 350 Pro". We use set_ssid_callback + strstr instead.
//
// Pairing tips for boAt Stone Stone 350 Pro (and similar):
//   - Turn OFF / unpair the speaker on your phone, or it may refuse the ESP32.
//   - Power the speaker and put it in pairing (often long-press power / BT).
//   - First scan after boot can take 15–45 s with Wi-Fi on (2.4 GHz shared).
//   - Watch Serial: a matching device prints as [BT scan] MATCH ...
//     (non-matching devices are not logged, to avoid flooding the UART).
//
// boAt Stone Stone 350 Pro: advertised name is usually like "boAt Stone Stone 350 Pro" or
// "Stone Stone 350 Pro". Substring "Stone 350 Pro" is specific enough for most rooms.
static const char* BT_SPEAKER_SUBSTRING = "Stone 350 Pro";
...
// With Wi‑Fi + TLS + Bluedroid A2DP on WROOM, 2 s often leaves ~4–8 KB free
// heap (OOM risk); 1 s is safer. Use 2–3+ on WROVER (PSRAM).
//   1 s = 64 KB utterance RAM
//   2 s = 128 KB
//   8 s = 512 KB (PSRAM only)
static const int    MAX_UTTERANCE_SECONDS = 1;
...
static const float    TRIGGER_SNR        = 1.55f;
static const float    RELEASE_SNR        = 1.5f;
...
// Loud rooms idle near 3k–6k RMS; capping at 4k lied to the SNR math
// (trigger = noise×SNR stayed unreachable). Allow the EMA to track reality.
static const float    NOISE_FLOOR_MAX    = 12000.0f;
...
// Smaller ring frees ~8 KB heap on WROOM; increase if TTS underruns (choppy audio).
static const size_t AUDIO_RING_BYTES   = 8 * 1024;
...
// True when any substring of `BT_SPEAKER_SUBSTRING` appears in the advertised
// Bluetooth name. Logs every A2DP-sink candidate so you can tune the string.
// Signature must match BluetoothA2DPSource::set_ssid_callback exactly:
// third argument is `int`, not int32_t, or the function pointer won't convert.
static bool btSpeakerSsids(const char* ssid, esp_bd_addr_t address, int rssi) {
  (void)address;
  if (!ssid || !ssid[0]) return false;
  bool ok = strstr(ssid, BT_SPEAKER_SUBSTRING) != nullptr;
  if (ok) log_bt_match(ssid, rssi);
  return ok;
}
...
// Retries forever instead of ESP.restart() (which caused rst:0xc boot loops and
// prevented A2DP from ever pairing). WL_CONNECT_FAILED (6) usually means wrong
// password; captive / enterprise SSIDs need different handling.
static void connectWifi() {
  ...
  for (;;) {
    ...
    const unsigned long attempt_ms = 30000;
    while (true) {
      ...
      if (millis() - start > attempt_ms) {
        LOGF("\nWiFi attempt timed out (status=%d). Retrying in 5 s "
             "(no reboot — check 2.4 GHz SSID/PWD).\n",
             (int)WiFi.status());
...
// Runs once after BT/I2S/tasks start so scanning can proceed while Wi-Fi joins.
...
static void allocateBuffers() {
  LOGF("Heap at boot: free=%u, largest=%u, psram_free=%u\n",
  ...
  if (!preroll_buf) {
    LOGLN("FATAL: preroll alloc failed");
  ...
  for (int s = MAX_UTTERANCE_SECONDS; s >= 1; s--) {
    ...
    if (a && b) {
      ...
      LOGF("utterance pool: 2 x %u bytes (%d s each). free=%u\n",
           (unsigned)bytes, s, (unsigned)ESP.getFreeHeap());
      if (s != MAX_UTTERANCE_SECONDS) {
        LOGF("NOTE: requested %d s but only %d s fit per slot.\n",
    ...
    LOGF("  %d s pair (%u bytes each) didn't fit, trying smaller...\n",
         s, (unsigned)bytes);
  }

  LOGLN("FATAL: could not allocate even 1 s utterance pool. Use ESP32-WROVER (PSRAM).");
  while (true) delay(1000);
}
...
static void finalizeUtterance(const char* reason) {
  ...
  if (utt_voiced_frames < MIN_VOICED_FRAMES) {
    LOGF("[utt] %s. voiced=%.0fms total=%.0fms -> DISCARD (<%dms)\n",
    ...
    LOGF("[utt] %s. voiced=%.0fms total=%.0fms -> queue slot %u\n",
         reason, voiced_ms, total_ms, (unsigned)utt_active_slot);
    if (xQueueSend(utt_queue, &job, 0) != pdTRUE) {
      LOGLN("[utt] utt_queue full, dropping utterance");
  ...
  for (;;) {
    ...
    if (vad_state == VAD_IDLE) {
      ...
      if (loud_enough && above_noise) {
        ...
        if (slot < 0) {
          LOGLN("[vad] both utterance slots busy, dropping trigger");
          // Do not EMA noise toward loud speech here — that pegs noise at NOISE_FLOOR_MAX
          // and makes triggers unreliable while STT/backlog clears.
          continue;
        }
        ...
        LOGF("[trigger] rms=%.0f peak=%u noise=%.0f snr=%.1fx slot=%d\n",
        ...
        if (++idle_log_counter >= NOISE_LOG_PERIOD) {
          idle_log_counter = 0;
          LOGF("[idle] noise=%.0f rms=%.0f peak=%u heap=%u maxblk=%u\n",
    ...
    // RMS only: in noisy spaces peak is almost always ≫ ABSOLUTE_MIN_PEAK, so
    // the old "|| peak ≥ min_peak" branch never let hangover run (no EOS).
    if (st.rms >= release_rms) {
      utt_voiced_frames++;
      utt_hangover_frames = 0;
    } else {
      utt_hangover_frames++;
...
static void net_task(void*) {
  ...
  for (;;) {
    ...
    if (WiFi.status() != WL_CONNECTED) {
      LOGLN("[net] WiFi dropped, reconnecting...");
      connectWifi();
    }
    ...
    LOGF("[net] POST /transcribe %u bytes (%.2fs)\n",
    ...
    if (transcript.length() == 0) {
      LOGF("[net] empty transcript in %lu ms (body=%s)\n",
           elapsed, body.c_str());
      continue;
    }
    LOGF("[net] transcript (%lu ms): %s\n", elapsed, transcript.c_str());
...
static void postChatAndStream(const String& merged) {
  if (WiFi.status() != WL_CONNECTED) {
    LOGLN("[chat] WiFi dropped, reconnecting...");
    connectWifi();
  }
  ...
  LOGF("[chat] POST /chat (%u text bytes)\n", (unsigned)body.length());
  unsigned long t0 = millis();
  int code = http.POST(body);
  unsigned long t_resp = millis() - t0;

  if (code <= 0) {
    LOGF("[chat] transport error %d (%s) in %lu ms\n",
  ...
  if (replyHdr.length()) {
    LOGF("[chat] reply preview (urlenc): %s\n", replyHdr.c_str());
  }

  WiFiClient* stream = http.getStreamPtr();
  if (!stream) {
    LOGLN("[chat] no body stream");
    http.end();
    return;
  }
  ...
  http.end();
  LOGF("[chat] done: %u PCM bytes (cl=%d, remain=%u), total %lu ms, first audio +%lu ms\n",
...
static void merge_task(void*) {
  ...
  for (;;) {
    ...
    if (got == pdTRUE) {
      ...
      last_at = now;
      fragments++;
      LOGF("[merge] +fragment (%u total): %s\n",
           (unsigned)fragments, pending.c_str());

      if (fragments >= MAX_FRAGMENTS) {
        LOGLN("[merge] MAX_FRAGMENTS reached -> flush");
...
void setup() {
  ...
  if (!audio_ring || !utt_queue || !txt_queue) {
    LOGLN("FATAL: queue/streambuffer alloc failed");
    while (true) delay(1000);
  }
  ...
  setupI2S();
  delay(500); // I2S settle before RF work
  ...
  a2dp_source.set_ssid_callback(btSpeakerSsids);
  a2dp_source.start();
  LOGF(
    "A2DP scanning (Wi-Fi + BT share 2.4 GHz — first connect can take 30–60 s).\n"
    "Matching any name containing substring: \"%s\"\n"
    "[BT scan] logs only when a matching speaker is seen (not every device).\n",
    BT_SPEAKER_SUBSTRING);

  // Tasks
  static const uint32_t STACK_VAD   = 6144;
  static const uint32_t STACK_NET   = 6144;
  static const uint32_t STACK_MERGE = 6144;
  xTaskCreatePinnedToCore(vad_task,   "vad",   STACK_VAD,   nullptr, 5, nullptr, 0);
  xTaskCreatePinnedToCore(net_task,   "net",   STACK_NET,   nullptr, 6, nullptr, 1);
  xTaskCreatePinnedToCore(merge_task, "merge", STACK_MERGE, nullptr, 3, nullptr, 0);
  static const uint32_t STACK_WIFI = 4096;
  xTaskCreatePinnedToCore(wifi_connect_task, "wifi", STACK_WIFI, nullptr, 4, nullptr, 1);
  ...
  LOGF("Heap after start: free=%u maxblk=%u\n", fh, mb);
  if (mb < 1536) {
    LOGLN(
      "WARN: maxblk very low — TLS POST may fail. Prefer ESP32-WROVER (PSRAM) or "
      "shrink AUDIO_RING_BYTES further.");
  } else if (fh < 20000) {
    LOGLN(
      "NOTE: free heap is low on WROOM + WiFi+BT+A2DP (often ~6–15 KB left); "
      "this is typical. If you see OOM or HTTPS errors, use WROVER or reduce buffers.");
  }
}
